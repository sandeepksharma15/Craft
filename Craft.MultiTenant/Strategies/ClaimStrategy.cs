                     ...
                     string authenticationScheme = "Bearer") : ITenantStrategy
{
    ...
    public async Task<string> GetIdentifierAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            var handler = (IAuthenticationHandler)ActivatorUtilities.CreateInstance(context.RequestServices, authScheme.HandlerType);
            ...
            return handlerResult.Principal is { } principal ? principal.FindFirst(_tenantKey)?.Value : null;
        }
        catch
        {
            return null;
