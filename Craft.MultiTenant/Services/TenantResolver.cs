...
public class TenantResolver<T> : ITenantResolver<T>, ITenantResolver where T : class, ITenant, IEntity, new()
{
    ...
    public async Task<ITenantContext<T>> ResolveAsync(HttpContext context)
    {
        ...
        foreach (var strategy in Strategies)
        {
            ...
            {
                identifier = null;
                _logger.LogInformation("Identifier \"{Identifier}\" is ignored for request at {RequestPath}",
                ...
                foreach (var store in Stores)
                {
                    ...
                    if (tenant is not null)
                    {
                        ...
                        result = new TenantContext<T>(tenant,
                            new TenantStore<T>(store, store.GetType()),
                            new TenantStrategy(strategy, strategy.GetType()));
