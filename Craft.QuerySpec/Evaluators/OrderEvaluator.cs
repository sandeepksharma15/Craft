...
public sealed class OrderEvaluator : IEvaluator
{
    ...
    {
        if (query.OrderBuilder.OrderDescriptorList.Count == 0)
            return queryable;

        if (query.OrderBuilder.OrderDescriptorList.Count(x => x.OrderType is OrderTypeEnum.OrderBy
                    or OrderTypeEnum.OrderByDescending) > 1)
            throw new ArgumentException("There is a duplicate order chain");

        IOrderedQueryable<T> orderedQueryable = null;

        foreach (var orderExpression in query.OrderBuilder.OrderDescriptorList)
